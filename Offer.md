...
engagement ends with the drift gate running in their own repository,
failing against their real drift and naming exactly what moved. The
scorecard is generated by the gate rather than typed by me, so "measured
again in three months" is a thing they can run rather than a promise I
made.
...
without me, which is a design brief for not needing Orin. A gate keeps
...
pounds" and becomes **make the gate pass**, which is bounded and
checkable.
...
**What the client gets:** The gate, installed and running in their
repository. The scorecard it generates. And the diagnosis: where the
...
a gate running in your own repo that tells you where your design system
and your code disagree, plus the diagnosis of what that's costing you.
...
findings matter, and leaving the script behind wired into your build so
...
the gate generate the scorecard, and walk the findings live because the
...
**The drift gate arrives in the Diagnostic and starts passing here.** As of
2026-08-28 the gate is installed during the Diagnostic, where its job is to
fail and keep naming what is broken. The Build is where it goes green and
becomes part of the working pipeline. The check compares their Figma file
against the tokens in their repo and fails when the two disagree, naming
exactly what moved. It also names both readings, because what
